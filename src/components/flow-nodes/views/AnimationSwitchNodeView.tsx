...
import { NumberField, SelectField, SectionTitle, StatusLine } from '../widgets'
import { useCanvasFill } from '../hooks/useCanvasFill'
import {
    animationSwitchDef, AnimationSwitchProcessor,
    type AnimationSwitchSnapshot,
} from '../../../node-engine/processors/animation-switch'
import {
    applyEasing,
    type SwitchProfile, type SwitchSide,
} from '../../../node-engine/processors/signal-switch'
    ...
    return (
        ...
        >
            ...
            <StatusLine tone="muted">
                wire ANIMATION into <code>a</code> &amp; <code>b</code>; EVENT into <code>event</code>.
                each event toggles A↔B with per-channel cross-fade using the matching profile.
            </StatusLine>
        </BaseNodeShell>
    )
})
...
const ChannelTable = memo(function ChannelTable({ readout }: ChannelTableProps) {
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', rowGap: 0.25 }}>
            ...
            {channels.length === 0 && (
                <Typography variant="caption" sx={{ color: COL_MUTED, opacity: 0.7 }}>
                    no channels — wire AnimationController into a or b.
                </Typography>
            )}
            {channels.map(c => (
