...
import { useCanvasFill } from './useCanvasFill'
import {
...
export const CombineSignalsNodeView = memo(({ id, data }: NodeProps & { data: PipelineNodeData }) => {
    ...
    return (
        <BaseNodeShell title="Combine" category={combineSignalsDef.category} inputs={combineSignalsDef.inputs} outputs={combineSignalsDef.outputs} minWidth={260}>
            <canvas
                ref={canvasRef}
                className="nodrag"
                style={{
