...
export class LogProcessor extends BaseProcessor {
    readonly def = logDef
    /* Not alwaysDirty: the engine already propagates dirty marks from
       upstream Effect (which IS alwaysDirty), so connected Log nodes
       tick every frame. When the input is unwired, Log stays idle —
       avoids spamming the view loop with no-op `setSnapshot(null)`
       calls and keeps the rAF pipeline lean. */
